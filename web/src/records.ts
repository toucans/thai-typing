...
// that could conflict.
import type { DictationRun, NewRun, Run, TypingRun } from './types.ts';
...
let cache: Run[] = [];      // the session's copy of this user's runs (also the offline fallback)
let loaded = false;         // has cache been filled from the server yet this session?
let pending: Run[] = [];    // runs whose POST failed; retried before the next load/save

export function currentUser(): string | null { return user; }
...
async function auth(endpoint: string, name: string): Promise<string> {
  ...
  user = String(body.user); // server-normalized (lowercase)
  localStorage.setItem(USER_KEY, user);
  return user;
}

export const login = (name: string): Promise<string> => auth('api/login', name);
export const createUser = (name: string): Promise<string> => auth('api/user', name);
...
export async function loadRuns(force = false): Promise<Run[]> {
...
export async function saveRun(run: NewRun, keepalive = false): Promise<void> {
  ...
  try {
    ...
    pending.push(full); // retried on the next load/save; lost if the tab closes
  }
...
export function starsFor(acc: number, cpm: number, baseline: number): number {
  let stars = 0;
  if (acc >= 0.80) stars = 1;
...
export function pbHistory(runs: Run[]): TypingRun[] {
  const speed = runs
...
// ข่าว runs are text runs with a 'ข่าว: ' name prefix; everything else with
// game === 'text' is a เรื่องอ่าน story.
const isNews = (r: TypingRun): boolean => (r.name || '').startsWith('ข่าว: ');
...
export function stats(runs: Run[]): Stats {
  const speed = runs.filter((r): r is TypingRun => r.game === 'speed');
  const starsByLevel = new Map<number, number>();
  let maxDone = 0;
  let pb = 0;
  let pbAt: string | null = null;
  let accPb = 0;
  const cpms: number[] = []; // rolling window source for each run's baseline
  for (const r of speed) {
    const win = cpms.slice(-10).sort((a, b) => a - b);
    const base = win.length >= 3 ? win[Math.floor(win.length / 2)] ?? 0 : 0;
    if (r.level) {
  ...
  const baseline = last.length >= 3 ? last[Math.floor(last.length / 2)] ?? 0 : 0;
  ...
  const recent = speed.filter((r) => Date.now() - new Date(r.t).getTime() < 30 * 864e5);
  const avg30 = recent.length
    ? recent.reduce((s, r) => s + r.cpm, 0) / recent.length : 0;
  const dictRuns = runs.filter((r): r is DictationRun => r.game === 'dictation');
  ...
  // order gives the current state — the same walk dictation.ts does at session
  // start to decide what opens the next round.
  const dictOwed = new Set<string>();
  ...
  const textRuns = runs.filter((r): r is TypingRun => r.game === 'text' && !isNews(r));
  const newsRuns = runs.filter((r): r is TypingRun => r.game === 'text' && isNews(r));
  ...
  return { starsByLevel, maxDone, pb, pbAt, accPb, baseline, streak, totalChars, totalSecs, avg30, dictWords, dictOwed, textsRead, textPb, newsRead, newsPb, newsChars, newsBySource, newsTitles };
}
